...
async function fetchWithRetry(url: string, options: FetchOptions = {}, maxRetries = 3): Promise<Response> {
...
async function fetchCourseData(): Promise<void> {
  try {
    ...
    if (errorMessage.includes('terminated') || errorMessage.includes('ECONNRESET')) {
