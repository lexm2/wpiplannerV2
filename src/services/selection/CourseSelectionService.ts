...
export class CourseSelectionService {
  ...
  async selectCourse(
    ...
    const { isRequired = false, validateBeforeAdd = true } = options;
  ...
  async setSelectedSection(
    ...
    try {
      ...
      if (
        sectionNumber !== null &&
        !Validators.validateSectionNumber(sectionNumber)
      ) {
        return {
          success: false,
  ...
  async clearAllComponents(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/require-await -- withBatch takes () => Promise<T>
  ...
  // eslint-disable-next-line @typescript-eslint/require-await -- awaited; paired with the ScheduleManagementService method
  async performHealthCheck(): Promise<{ healthy: boolean; issues: string[] }> {
    ...
    try {
      ...
      if (!validation.valid) {
        issues.push(
          `Data validation: ${validation.errors.length} errors found`,
        );
      }
    } catch (error) {
  ...
  // eslint-disable-next-line @typescript-eslint/require-await -- awaited by performHealthCheck
