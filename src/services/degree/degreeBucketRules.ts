...
function resolveYear<T>(byYear: Record<number, T>, year: number | null): T | undefined {
 ...
 */
export function getSheetRules(major: string, classYear: number | null): SheetRules {
...
export function getBucketRule(
    ...
    const candidates = getSheetRules(major, classYear).filter(r => r.category === category);
...
export function bucketRuleToCriteria(rule: DegreeBucketRule): DegreeBucketFilterCriteria {
    return {
