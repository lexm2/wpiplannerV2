...
export function matchPlannedCourses(
  ...
  for (const pc of planned) {
    ...
    const course: Course | null = findCatalogCourse(pc.code, ay, departments, {
    ...
    if (term) {
      const groups = (course.lectures ?? []).filter(g =>
        termMatches(g.section.computedTerm, term),
      );
      if (groups.length === 1) {
        const [group] = groups;
        selected.lecture = group.section;
        if (group.compatibleDiscussions.length === 1)
          selected.discussion = group.compatibleDiscussions[0];
        if (group.compatibleLabs.length === 1)
          selected.lab = group.compatibleLabs[0];
      }
    }
  ...
  return {
    selections,
    year,
    stats: { matched: selections.length, autoSectioned, pinnedOnly, unmatched },
  };
}
