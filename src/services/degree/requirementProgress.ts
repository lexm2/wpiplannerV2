...
const RANK: Record<RequirementStatus, number> = { not_satisfied: 0, in_progress: 1, satisfied: 2 };
const EMPTY_CAP = 12;
...
function anchor(computed: RequirementStatus, base: RequirementStatus): RequirementStatus {
 ...
 */
function bucketTiles(tiles: ProgressTile[]): { planned: number; plannedCount: number; schedule: number; scheduleCount: number } {
    const byCode = new Map<string, { kind: 'planned' | 'schedule'; credits: number }>();
    for (const t of tiles) {
        const cur = byCode.get(t.code);
        if (!cur) byCode.set(t.code, { kind: t.kind, credits: t.credits });
        else if (cur.kind === 'schedule' && t.kind === 'planned') byCode.set(t.code, { kind: 'planned', credits: t.credits });
    }
    let planned = 0, plannedCount = 0, schedule = 0, scheduleCount = 0;
    for (const v of byCode.values()) {
        if (v.kind === 'planned') { planned += v.credits; plannedCount++; }
        else { schedule += v.credits; scheduleCount++; }
    }
...
function makeSegments(earned: number, planned: number, schedule: number, total: number, satisfied: boolean): ProgressSegments {
    ...
    return { earned: satisfied ? Math.max(ew, 1 - pw - sw) : ew, planned: pw, schedule: sw };
}

export function effectiveProgress(req: Requirement, tiles: ProgressTile[]): EffectiveProgress {
    const earnedCredits = req.appliedCourses.filter(c => !c.isInProgress).reduce((s, c) => s + c.credits, 0);
    ...
    if (required != null && required > 0) {
        ...
        const computed: RequirementStatus = covered >= required ? 'satisfied' : covered > 0 ? 'in_progress' : 'not_satisfied';
        const status = anchor(computed, req.status);
        const creditsRemaining = round2(Math.max(0, required - covered));
        const segments = makeSegments(earnedCredits, planned, schedule, required, status === 'satisfied');
        return {
            status,
            ...
            emptySlots: status === 'satisfied' ? 0 : Math.min(EMPTY_CAP, Math.max(1, Math.ceil(creditsRemaining / 3))),
        };
    }
    ...
    if (req.coursesRemaining != null) {
        ...
        const computed: RequirementStatus =
            requiredCourses > 0 && have >= requiredCourses ? 'satisfied' : have > 0 ? 'in_progress' : 'not_satisfied';
        const status = anchor(computed, req.status);
        const coursesRemaining = Math.max(0, requiredCourses - have);
        const segments = makeSegments(completedCount, plannedCount, scheduleCount, requiredCourses, status === 'satisfied');
        return {
            status,
            fraction: requiredCourses > 0 ? segments.earned + segments.planned + segments.schedule : null,
            segments,
            creditsRemaining: null,
            coursesRemaining,
            emptySlots: status === 'satisfied' ? 0 : Math.min(EMPTY_CAP, coursesRemaining),
        };
    }

    // No numeric target (e.g. "Minimum Combination Required").
    const computed: RequirementStatus = plannedCount + scheduleCount > 0 ? 'in_progress' : req.status;
    const status = anchor(computed, req.status);
    return {
        status,
        fraction: status === 'satisfied' ? 1 : null,
        segments: status === 'satisfied' ? { earned: 1, planned: 0, schedule: 0 } : EMPTY_SEGMENTS,
        creditsRemaining: null,
        coursesRemaining: null,
        emptySlots: status === 'satisfied' ? 0 : req.appliedCourses.length || plannedCount + scheduleCount ? 0 : 1,
    };
}
