...
export function computePlacements(
    ...
    for (const it of items) {
        ...
        const tile: DegreeTile = moved
            ? { ...it, moved: true, confidence: it.kind === 'schedule' ? 'manual' : it.confidence }
            : it;
...
const codeOf = (course: Course): string => `${course.departmentAbbr} ${course.number}`;
 ...
 */
export function inferRequirementDepartments(req: Requirement, departments: Department[]): string[] {
    if (req.appliedCourses.length) {
        ...
        return [...counts.entries()].sort((a, b) => b[1] - a[1]).map(([dept]) => dept);
    }
    ...
    switch (req.category) {
        ...
        case 'major_specific': {
            ...
            const match = name ? departments.find(d => d.name.trim().toLowerCase() === name) : undefined;
