...
const COL = { requirement: 0, status: 1, remaining: 2, course: 3, period: 4, credits: 5, grade: 6 } as const;
...
export function parseAcademicPeriod(raw: string): AcademicPeriod | null {
    ...
    if (term) {
        return { year: parseInt(term[1], 10), season: term[2] as 'Fall' | 'Spring', term: term[3], raw: s };
    }
    const sem = /^(\d{4})\s+(Fall|Spring)\s+Semester$/.exec(s);
    if (sem) {
        return { year: parseInt(sem[1], 10), season: sem[2] as 'Fall' | 'Spring', term: null, raw: s };
    }
...
export function parseRemaining(raw: string): { creditsRemaining: number | null; coursesRemaining: number | null } {
    const s = raw?.trim() ?? '';
    const credit = /^Minimum\s+([\d.]+)\s+Credit\(s\)$/i.exec(s);
    if (credit) return { creditsRemaining: parseFloat(credit[1]), coursesRemaining: null };
    const course = /^Minimum\s+(\d+)\s+Course\(s\)$/i.exec(s);
    if (course) return { creditsRemaining: null, coursesRemaining: parseInt(course[1], 10) };
 ...
 */
export function parseCourseString(raw: string, period: AcademicPeriod | null, credits: number, grade: string | null): AppliedCourse | null {
    let s = raw?.trim();
    if (!s) return null;
...
export function classifyCategory(rawName: string): RequirementCategory {
    ...
    if (/Interactive Qualifying Project Completion/i.test(n)) return 'iqp_completion';
...
export function parseRequirementName(rawName: string): { scope: string; name: string; creditsRequired: number | null } {
    // Match "<n> Credits" or "<n> or <m> Credits", taking the lower bound.
    const creditMatch = /(\d+(?:\.\d+)?)(?:\s+or\s+\d+(?:\.\d+)?)?\s+Credits?/i.exec(rawName);
    ...
    if (/^WPI\s+/i.test(prefix)) {
        return { scope: 'WPI', name: prefix.replace(/^WPI\s+/i, '').trim(), creditsRequired };
    }
    ...
    if (dashIdx >= 0) {
        return { scope: prefix.slice(0, dashIdx).trim(), name: prefix.slice(dashIdx + 3).trim(), creditsRequired };
    }
    return { scope: prefix, name: prefix, creditsRequired };
}

/** Derive "Computer Science" / "BS" from the MQP requirement's parenthetical. */
function deriveMajorDegree(requirements: Requirement[]): { major: string; degree: string } {
    ...
    for (const req of requirements) {
        if (req.scope && req.scope !== 'WPI') counts.set(req.scope, (counts.get(req.scope) ?? 0) + 1);
    }
    let major = '';
    let best = 0;
    for (const [scope, c] of counts) if (c > best) { best = c; major = scope; }
    return { major, degree: '' };
}
...
export function parseAcademicProgress(rows: string[][]): StudentRecord {
    ...
    let headerRow = rows.findIndex(r => (r[COL.requirement] ?? '').trim() === 'Requirement');
    ...
    for (let i = headerRow + 1; i < rows.length; i++) {
        ...
        if (courseStr && courseStr.toLowerCase() !== 'none') {
            ...
            const course = parseCourseString(courseStr, period, credits, grade && grade.toLowerCase() !== 'none' ? grade : null);
    ...
    const required = requirements.find(r => r.category === 'total_credits')?.creditsRequired ?? null;
    const credits: CreditTotals = {
        earned: round2(courses.filter(c => !c.isInProgress).reduce((s, c) => s + c.credits, 0)),
        inProgress: round2(courses.filter(c => c.isInProgress).reduce((s, c) => s + c.credits, 0)),
        transfer: round2(courses.filter(c => c.isTransfer).reduce((s, c) => s + c.credits, 0)),
        required,
    };
