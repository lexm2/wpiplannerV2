...
export class CourseDataService {
  ...
  private parseJSONData(jsonData: {
    ...
    if (!jsonData.departments || !Array.isArray(jsonData.departments)) {
      ...
      throw new Error(
        'Invalid JSON data structure - missing departments array',
      );
    }

    const realDepartments = this.parseConstructedDepartments(
      jsonData.departments,
    );
    const latestYear = Math.max(
      ...realDepartments.flatMap(d => d.courses.map(c => c.academicYear ?? 0)),
    );
  ...
  private parseConstructedDepartments(
    departments: RawDepartment[],
  ): Department[] {
      ...
      department.courses = deptData.courses.map((courseData: RawCourse) => {
        ...
        if (seenIds.has(courseId)) {
          ...
          if (STRICT_VALIDATION) {
            ...
            logger.error(
              `DUPLICATE ID: "${courseId}" for ${department.abbreviation}${courseData.number}`,
            );
    ...
    if (duplicatesFixed > 0) {
      logger.warn(
        `Course ID deduplication: fixed ${duplicatesFixed} of ${totalCoursesProcessed} courses; affected IDs: [${Array.from(duplicateIds).join(', ')}]`,
      );
    }
  ...
  private parseLectureGroups(lectureGroups: RawLectureGroup[]): LectureGroup[] {
    return lectureGroups.map(groupData => {
      ...
      const compatibleDiscussions = this.parseConstructedSections(
        groupData.compatibleDiscussions || [],
      );
      const compatibleLabs = this.parseConstructedSections(
        groupData.compatibleLabs || [],
      );
  ...
  private parsePeriodType(typeString: string): PeriodType {
    ...
    switch (normalizedType) {
      ...
      default:
        logger.warn(
          `Unknown period type: "${typeString}", defaulting to Lecture`,
        );
        return PeriodType.LECTURE;
    }
  }
  ...
  searchCourses(query: string, departments?: string[]): Course[] {
    ...
    for (const dept of this.scheduleDB.departments) {
      if (
        ...
        !departments.includes(dept.abbreviation.toLowerCase())
      ) {
        continue;
      }
  ...
  getDiscussionsForLecture(course: Course, lectureSection: Section): Section[] {
    ...
    const lectureGroup = course.lectures.find(
      lg => lg.section.crn === lectureSection.crn,
    );
  ...
  isLabOnlyCourse(course: Course): boolean {
    return (
      ((!course.lectures || course.lectures.length === 0) &&
        course.standaloneLabs &&
        course.standaloneLabs.length > 0) ||
      false
    );
  }
  ...
  async addTutorialDepartment(): Promise<void> {
    if (!this.scheduleDB) return;
    const response = await fetch('./tutorial-courses.json', {
      cache: 'no-cache',
    });
    ...
    const parsedCourses = json.courses.map(raw => {
      ...
      const standaloneLabs = raw.standaloneLabs
        ? this.parseConstructedSections(raw.standaloneLabs)
        : undefined;
      const course: Course = {
        id: raw.id,
        number: raw.number,
    ...
    this.scheduleDB = {
      ...
      departments: [
        tutDept,
        ...this.scheduleDB.departments.filter(d => d.abbreviation !== 'TUT'),
      ],
    };
    this.notifyDataRefreshed();
  }
