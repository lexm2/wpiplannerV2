...
export const reduceMotion = typeof matchMedia !== 'undefined'
 ...
 */
export function slideFade(node: Element, { duration = 260, delay = 0 } = {}): TransitionConfig {
...
export function slideX(
    ...
    return {
        ...
        delay,
        easing: cubicOut,
        css: (t: number, u: number) => `transform: translateX(${u * from * 100}%); opacity: ${t};`,
    };
}
...
export function scaleFade(
    ...
    return {
        ...
        delay,
        easing: cubicOut,
        css: (t: number) => `transform: scale(${from + (1 - from) * t}); opacity: ${t};`,
    };
}
