<script lang="ts">
  ...
  async function buildSchedule(): Promise<void> {
    ...
    try {
      ...
      buildResult = err instanceof Error ? err.message : 'Failed to build schedule.';
    } finally {
      building = false;
    }
  }
  ...
  const overallPct = $derived(
    required && required > 0 ? Math.min(100, Math.round((record.credits.earned / required) * 100)) : null
  );

  const title = $derived(
    [record.major, record.degree].filter(Boolean).join(' · ') || 'Academic Progress'
  );
  ...
  async function onInputChange(e: Event): Promise<void> {
    ...
    input.value = '';
    if (file) {
      try { await degreeImportService.importFromFile(file); } catch { /* surfaced in state */ }
    }
  }
</script>
...
<section class="degree-summary">
  <div class="degree-summary-head">
    ...
    <div class="degree-summary-actions">
      <button type="button" class="btn" onclick={() => fileInput?.click()}>Re-import</button>
      <button type="button" class="btn btn-ghost" onclick={() => degreeImportService.clear()}>Clear</button>
      <input bind:this={fileInput} type="file" accept=".xlsx" class="degree-file-input" onchange={onInputChange} />
    </div>
  </div>

  {#if overallPct !== null}
    <div class="degree-progress">
      <div class="degree-progress-bar"><div class="degree-progress-fill" style:width="{overallPct}%"></div></div>
      <span class="degree-progress-label">{overallPct}% of {required} credits</span>
    </div>
  {/if}

  <div class="degree-credit-stats">
    <div class="degree-stat"><span class="degree-stat-value">{record.credits.earned}</span><span class="degree-stat-label">Earned</span></div>
    <div class="degree-stat"><span class="degree-stat-value">{record.credits.inProgress}</span><span class="degree-stat-label">In progress</span></div>
    <div class="degree-stat"><span class="degree-stat-value">{record.credits.transfer}</span><span class="degree-stat-label">Transfer</span></div>
