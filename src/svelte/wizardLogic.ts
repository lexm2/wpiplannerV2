...
export function determineAvailableSteps(
    ...
    if (isLabOnly) {
        ...
        if (lectures.length > 0) {
            ...
            if (selectedLecture) {
                return computeStepsForLecture(course, courseDataService, selectedLecture);
            }
...
export function getOptionsWithFilterInfo(
    ...
    if (step === 'lecture') {
        if (courseDataService.isLabOnlyCourse(course)) {
            ...
            sections = courseDataService.getLecturesForCourse(course).map(lg => lg.section);
        }
    ...
    if (filterService && !filterService.isEmpty() && sections.length > 0) {
        const passing = new Set(filterService.apply([course]).map(fs => fs.section.crn));
        return { filtered: sections.filter(s => passing.has(s.crn)), totalBeforeFilter };
    }

    return { filtered: sections, totalBeforeFilter };
}

function getAllDiscussionsForCourse(course: Course, cds: CourseDataService): Section[] {
...
function getAllLabsForCourse(course: Course, cds: CourseDataService): Section[] {
    const all = new Map<number, Section>();
    for (const lg of cds.getLecturesForCourse(course)) {
...
export function groupSectionsByTerm(sections: Section[]): Array<{ term: string; sections: Section[] }> {
...
export function getRmpRatingClass(rating: string): 'excellent' | 'good' | 'poor' {
 ...
 */
export function describeActiveFilters(filterService: FilterService | null): string[] {
    ...
    for (const filter of filterService.getActiveFilters()) {
        if (filter.id === 'periodRmpRating' && filter.criteria) {
            const c = filter.criteria as {
                ...
                minWouldTakeAgain?: number; maxWouldTakeAgain?: number;
            };
            const parts: string[] = [];
            if ((c.minRating ?? 0) > 0 || (c.maxRating ?? 5) < 5) {
                parts.push(`${(c.minRating ?? 0).toFixed(1)}-${(c.maxRating ?? 5).toFixed(1)} rating`);
            }
            if ((c.minDifficulty ?? 0) > 0 || (c.maxDifficulty ?? 5) < 5) {
                parts.push(`${(c.minDifficulty ?? 0).toFixed(1)}-${(c.maxDifficulty ?? 5).toFixed(1)} difficulty`);
            }
            if ((c.minWouldTakeAgain ?? 0) > 0 || (c.maxWouldTakeAgain ?? 100) < 100) {
                parts.push(`${c.minWouldTakeAgain ?? 0}-${c.maxWouldTakeAgain ?? 100}% retake`);
            }
