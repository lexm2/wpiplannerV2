<script lang="ts">
	...
	function currentWidth(handle: HTMLElement): number {
		...
		const raw = getComputedStyle(document.documentElement).getPropertyValue(config.cssVar);
	...
	function onKeyDown(e: KeyboardEvent): void {
		...
		localStorage.setItem(config.storageKey, String(Math.round(currentWidth(handle))));
		e.preventDefault();
	}
</script>
