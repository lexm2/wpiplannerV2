...
export function animateFindDot(selector: string): void {
    ...
    overlay.animate([{ opacity: 0 }, { opacity: 1 }], { duration: reduceMotion ? 0 : 300, fill: 'forwards' });
    ...
    growIn.finished
        .then(() => dot.animate(
            ...
            { duration: reduceMotion ? 0 : 500, easing: 'cubic-bezier(0.76, 0, 0.24, 1)', fill: 'forwards' }
        ).finished)
        .then(() => {
            // The 500ms delay is the hold on the target before fading away.
            overlay.animate([{ opacity: 1 }, { opacity: 0 }], { duration: reduceMotion ? 0 : 300, delay: 500, fill: 'forwards' });
            return dot.animate(
                [
                    { transform: `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px)) scale(1)` },
                    { transform: `translate(calc(-50% + ${dx}px), calc(-50% + ${dy}px)) scale(0)` },
                ],
                { duration: reduceMotion ? 0 : 300, delay: 500, easing: 'ease-in', fill: 'forwards' }
            ).finished;
        })
