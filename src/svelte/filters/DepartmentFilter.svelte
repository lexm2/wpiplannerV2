    ...
    if (isCategoryMode) {
      return CATEGORY_ORDER.filter(c => c !== 'Other').map(category => {
        const depts = allDepartments.filter(
          d => getDepartmentCategory(d) === category,
        );
        const selected = depts.filter(d => activeDepartments.includes(d));
        const allSelected =
          depts.length > 0 && selected.length === depts.length;
  ...
  const visibleItems = $derived.by<Item[]>(() => {
    ...
    return items.filter(it =>
      isCategoryMode
        ...
        : departmentMatchesSearch(it.value, q),
    );
  });
  ...
  function applyFromSelection(values: string[]): void {
    ...
    if (isCategoryMode) {
      values.forEach(category => {
        departments.push(
          ...allDepartments.filter(d => getDepartmentCategory(d) === category),
        );
      });
    } else {
      departments = values;
    ...
    if (departments.length > 0)
      filterService.addFilter('department', { departments });
