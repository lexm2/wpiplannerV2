<script lang="ts">
  ...
  const hiddenCount = $derived(
    optionInfo.totalBeforeFilter - optionInfo.filtered.length,
  );
  ...
  function clearFilters(): void {
    ...
    const activeYear = filterService.getCriteria<{ year?: number }>(
      'academicYear',
    )?.year;
    filterService.resetFilters(
      typeof activeYear === 'number' ? activeYear : undefined,
    );
  }
...
>
  ...
  {#if availableSteps.length > 1}
    <div class={styles['wizard-breadcrumbs']}>
      {#each availableSteps as step, i (step)}
        {#if i > 0}
          <span class={styles['breadcrumb-arrow']}
            >{@html getInlineSVG(
              'ARROW_BAR_RIGHT',
  ...
  <div class={styles['wizard-content']}>
    {#key currentStep}
      ...
      >
        ...
        {:else}
          <h3 class={styles['wizard-step-title']}>
            {STEP_TITLES[currentStep]}
          </h3>
          {#each termGroups as group (group.term)}
            <div class={styles['wizard-term-separator']}>
              {getTermName(group.term)}
            </div>
              ...
              {#each group.sections as section, i (section.crn)}
                ...
                {@const rmp =
                  professor !== 'Not Assigned'
                    ? rateMyProfessorService.getRatingDisplay(professor)
                    : null}
                {@const rmpUrl =
                  professor !== 'Not Assigned'
                ...
                <div
                  ...
                  onkeydown={e =>
                    (e.key === 'Enter' || e.key === ' ') &&
                    (e.preventDefault(), selectSection(section))}
                  onmouseenter={() => hoverPreview(section)}
                  onmouseleave={clearHoverPreview}
                >
                  <div class={styles['section-card-header']}>
                    <span class={styles['section-card-number']}
                      >{section.number}</span
                    >
                  </div>
                  ...
                  {:else}
                    ...
                    <div class={styles['section-card-location']}>
                      {period?.location || 'TBA'}
                    </div>
                  {/if}
                  ...
                  <div class={styles['section-card-professor']}>
                    {#if rmpUrl}
                      <a
                        ...
                        class="professor-link">{professor}</a
                      >
                    {:else}
                      {professor}
                    {/if}
                    {#if rmp}
                      <div
                        class={styles['rmp-badge']}
                        title="Rate My Professor: {rmp.rating}/5.0 ({rmp.numRatings} ratings)"
                      >
                        <span
                          class={[
                            styles['rmp-rating'],
                            styles[getRmpRatingClass(rmp.rating)],
                          ]}>★ {rmp.rating}</span
                        >
                        <span class={styles['rmp-details']}>
                          {rmp.difficulty}/5 difficulty{rmp.wouldTakeAgain
                            ? ` • ${rmp.wouldTakeAgain} would take again`
                            : ''}
                        </span>
                      </div>
                    {/if}
                  ...
                  <div class={styles['section-card-footer']}>
                    <span
                      class={[
                        styles['section-card-seats'],
                        { [styles['full']]: section.seatsAvailable === 0 },
                      ]}
                    >
                      {getSeatsInfo(section)}
                    </span>
                    <span class={styles['section-card-crn']}
                      >CRN: {section.crn}</span
                    >
                  </div>

                  {#if selected}<div
                      class={styles['section-card-selected-badge']}
                    >
                      ✓ Selected
                    </div>{/if}
                </div>
  ...
  <div class={styles['wizard-footer']}>
    ...
    <button
      class={[styles['wizard-btn'], styles['wizard-btn-text']]}
...
{#snippet filteredNotice()}
  {#if yearMismatch}
    <div class={styles['wizard-filtered-notice']}>
      <span class={styles['wizard-filtered-notice-text']}>
        This course is from {yearMismatch.courseYear}-{yearMismatch.courseYear +
          1} but you're viewing
  ...
  {:else}
    <div class={styles['wizard-filtered-notice']}>
      <span class={styles['wizard-filtered-notice-text']}>
        {hiddenCount}
        {hiddenCount === 1 ? 'section' : 'sections'} hidden by filters
      </span>
      <button
        class={styles['wizard-filtered-notice-btn']}
