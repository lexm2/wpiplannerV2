...
export function applyPreviewOverlay(
  ...
  if (idx >= 0) {
    return courses.map((sc, i) =>
      i === idx ? { ...sc, selected: selections } : sc,
    );
  }
...
function sectionDayRanges(
  ...
  WEEKDAYS.forEach((day, dayIndex) => {
    ...
    for (const p of periods) {
      startMin = Math.min(
        startMin,
        p.startTime.hours * 60 + p.startTime.minutes,
      );
...
export function buildTermBlocks(
  ...
  for (const [dayIndex, daySolids] of solidByDay) {
    const regions = new Map<
      string,
      { start: number; end: number; labels: Set<string> }
    >();
  ...
  for (const [dayIndex, idxs] of packByDay) {
    const layout = packColumns(
      idxs.map(i => ({ startMin: pack[i].startMin, endMin: pack[i].endMin })),
    );
    idxs.forEach((i, k) => {
      const { left, width } = packedWidth(
        dayIndex,
        layout[k].col,
        layout[k].cols,
      );
