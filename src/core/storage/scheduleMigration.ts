 ...
 */
export function migrateStoredSchedule(
  raw: unknown,
  ...
  if (
    ...
    storedVersion >= SCHEDULE_SCHEMA_VERSION
  ) {
  ...
  let changed = 0;
  // Annotated: Array.isArray narrows `unknown` to `any[]`.
  ...
  if (changed > 0) {
    logger.log(
      `Migrated ${changed} stored course(s) in schedule ${String(schedule.id)}`,
    );
  }
