...
export class DegreeBucketFilter implements SectionBasedFilter {
    ...
    readonly description = 'Restrict courses to those eligible for a degree requirement bucket';
    readonly priority = 24;

    apply(sections: FilterableSection[], criteria: DegreeBucketFilterCriteria): FilterableSection[] {
        ...
        const allowed = new Set(criteria.allowedDepartments.map(d => d.toUpperCase()));
        ...
        return sections.filter(fs => {
            ...
            if (hasExclusions && excluded.has(`${dept} ${fs.course.number}`)) return false;
            return true;
        });
    }
    ...
    isValidCriteria(criteria: unknown): criteria is DegreeBucketFilterCriteria {
        ...
        return Array.isArray(c.allowedDepartments) &&
    ...
    getDisplayValue(criteria: DegreeBucketFilterCriteria): string {
        ...
        const excl = criteria.excludedCourses.length ? `, ${criteria.excludedCourses.length} excluded` : '';
