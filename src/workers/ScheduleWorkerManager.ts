...
import { SmartScheduler, type SchedulerInput } from '../services/scheduling/SmartScheduler';
...
export class ScheduleWorkerManager {
    ...
    async generate(input: SchedulerInput, settings: AutoScheduleSettings, maxResults: number): Promise<ScheduleResult[][]> {
    ...
    private executeFallback(input: SchedulerInput, settings: AutoScheduleSettings, maxResults: number): ScheduleResult[][] {
        ...
        schedules.sort((a, b) => scorer.score(b, effectiveSettings) - scorer.score(a, effectiveSettings));
        return schedules;
    }
