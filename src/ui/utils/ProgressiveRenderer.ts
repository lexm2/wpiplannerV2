...
export class ProgressiveRenderer {
    ...
    renderCourseList(
        ...
        if (isLoadMore) {
            ...
            for (const course of courses) {
                ...
                const el = this.createCourseListItem(course, isSelected, isBookmarked);
                elementToCourseMap.set(el, course);
    ...
    renderCourseGrid(
        ...
        if (isLoadMore) {
            ...
            for (const course of courses) {
                const isSelected = courseSelectionService.isCourseSelected(course);
                const isBookmarked = stateManager.isBookmarked(course.id);
                const el = this.createCourseGridItem(course, isSelected, isBookmarked);
                elementToCourseMap.set(el, course);
                courseGridContainer.appendChild(el);
            }
        } else {
            const courseGridContainer = document.createElement('div');
            courseGridContainer.className = 'course-grid';
            ...
            container.innerHTML = '';
            container.appendChild(courseGridContainer);
        }

        this.options.onComplete?.(courses.length);
    }

    private createCourseListItem(course: Course, isSelected: boolean, isBookmarked: boolean): HTMLElement {
        ...
        item.className = `course-item${isSelected ? ' selected' : ''}`;
    ...
    private createCourseGridItem(course: Course, isSelected: boolean, isBookmarked: boolean): HTMLElement {
        const hasWarning = this.courseHasWarning(course);

        const item = document.createElement('div');
        item.className = `course-card${isSelected ? ' selected' : ''}`;
        item.dataset.courseId = course.id;
        item.innerHTML = `
