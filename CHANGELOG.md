...
### Added
- Auto-schedule now picks the schedule with the least wasted time between classes. Schedules are generated by first deciding which term each course belongs to, then finding the section combination within each term that minimizes daily gaps. This replaces the old brute-force approach and produces meaningfully better results.
