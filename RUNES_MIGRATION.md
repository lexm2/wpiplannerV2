...
### ✅ Imperative view-refresh teardown (Phase 12E.8) — DONE
- Deleted `MainController.refreshCurrentView` (a no-op since 12E.5) and all its callers, since every view is now a reactive Svelte component: `ViewToggle` lost its `onSelect` prop (`CourseList` derives layout from the `uiState.currentView` rune); `onDataRefreshed` was removed from the `AppBootstrap.setupCourseDataSubscriptions` contract + its call (the post-sync `appState.loadedDepartments` reassignment drives every view); `refreshUI` was deleted and `handleUndo`/`handleRedo` reduced to `.catch()` only; `initializeDefaultDepartmentView` + its startup call were removed. Playwright-verified: fresh load shows all courses, the view toggle flips list↔grid reactively with no callback, undo/redo round-trip cleanly; 0 console errors.

## PHASE 13 — App.svelte root shell + delete MainController + full bridge teardown (FINAL, staged 13A–13D)

### ✅ 13A — Decouple the tutorial from MainController — DONE
- `setupTutorial.ts` + `TutorialStateMachine.ts` no longer take/hold a `MainController`; every `mainController.<x>()` call is replaced by the direct service/rune call the wrapper delegated to: `closeWizard`/`openWizardForCourse` → `componentWizardService.closeComponentWizard()`/`openComponentWizard(...)` (with the `getSelectedCourses().find(...)` lookup inlined); `openSchedulePicker` → `uiStateManager.modalOpened('schedule-picker')`; `openFilterModal` → `modalState.filter = { mode: 'filter' }; modalOpened('filter-modal')`; `openAutoSchedule*` → `autoScheduleService.*`; `navigateSchedulePickerToTab`/`refreshPlannerFilterUI` → `modalState.schedulePickerTab`/`filterRefreshTick++`; `syncCourseSelectionUI` calls deleted (already a no-op). `main.ts` now calls `setupTutorial(services)`. **Deleted** MainController's now-unused public wrappers (`closeWizard`, `openWizardForCourse`, `jumpWizardToStep`, `navigateSchedulePickerToTab`, `openAutoSchedule`/`openAutoScheduleIntro`/`openAutoScheduleFilter`, `updateAutoScheduleIntroTerms`, `refreshAutoScheduleFilterUI`, `refreshPlannerFilterUI`, `syncCourseSelectionUI`, `runAutoSchedule`) + the now-unused `WizardStep` import; KEPT `openSchedulePicker`/`openFilterModal` (still used by the settings-menu/schedule-picker-btn click + FilterButtons/ScheduleFilterButton mount props). Playwright-verified all 5 rewired paths through real tutorials (wizard open, filter-modal, schedule-picker, auto-schedule-intro, auto-schedule-filter) with 0 app console errors; profile reverted (My Schedule, AB1531/AB1532 `lec: null`, no TUT dept). tsc 0 / build clean / 6-6 tests.

### ✅ 13B — Extract non-UI bootstrap out of MainController — DONE
- Moved all non-UI startup out of the MainController constructor/`init()` into `AppBootstrap`: `createServices` now also constructs `colorService` (`CourseColorService`) + `autoScheduleOrchestrator` (`AutoScheduleOrchestrator`) and returns them on the `ServiceContainer`; new `AppBootstrap.initStandaloneServices(services)` does the 4 `*Service.init(...)`; new async `AppBootstrap.startApp(services)` does `initializeAsyncServices` + saved theme + `setCalendarEventProvider` + `orchestrator.setupCourseSelectionChangeListener` + `setupWindowUnloadHandler` + the first-visit welcome-tutorial autostart (with the old try/catch error handling). `main.ts` is now the bootstrap sequencer: `createServices → initStandaloneServices → setupCourseDataSubscriptions → initializeFilters → new MainController → setupTutorial → startApp`. **Deleted dead code**: `setupCourseSelectionListener` + `refreshSelectionUI` + `updateSelectedCoursesState` + the `previousSelectedCoursesMap` field + the `SelectionSnapshot` import (all write-only/no-op). MainController is now ONLY: the ~16 component mounts (reading `services.colorService`/`services.autoScheduleOrchestrator`) + DOM glue (settings menu, resizable, swipe, schedule-picker-btn, keyboard, the 2 live watchers `setupScheduleChangeListener`/`setupPageNavigationListener`). FilterModal's `getDepartments` thunk now reads `appState.loadedDepartments` directly, so `setupCourseDataSubscriptions` dropped its `{setAllDepartments,onDataLoaded}` callbacks + MainController dropped its `allDepartments` field. Behavior-preserving (bootstrap order unchanged). tsc 0 / build clean / 6-6 tests; Playwright: app boots, 200 courses + departments load, services on container, page toggle (planner grid ↔ schedule flex, 2 sidebar items + 4 grids), undo/redo/settings present, 0 console errors; with a valid active schedule the AB1531/AB1532 selection loads. (NOTE: discovered a **pre-existing** startup race in `ScheduleManagementService.initializeDefaultScheduleIfNeeded` / `profileStateManager.loadFromStorage` — under Playwright's worker timing, `loadFromStorage` resolves before its schedules populate, so an empty default "My Schedule" is created each load; reproduced identically on the committed 13A code via git-stash A/B, so NOT a 13B regression; untouched by Phase 13. The user's real schedule is preserved in IndexedDB.)

### ✅ 13C — `App.svelte` root shell; delete MainController; collapse index.html — DONE
- Created **`src/svelte/App.svelte`** — the declarative root shell rendering the full header + `#planner-page` + `#schedule-page` and every child component (`DepartmentSidebar`/`SearchBar`/`FilterButtons`/`ViewToggle`/`CourseList`/`SelectedCoursesPanel`/`CourseDescription`/`PageTabs`/`UndoRedoButtons`/`ThemeSelector`/`SettingsMenu`/`CalendarEventsButton`/`ScheduleFilterButton`/`ClearAllSectionsButton`/`ScheduleSidebar`/`WizardHost`/`AutoScheduleControls`/`ScheduleGrids`) **declaratively** with the same props MainController passed to `mount()`. Page regions keep BOTH pages mounted and toggle `style:display` (`planner → grid|none`, `schedule → flex|none`) off `uiState.currentPage` — no `{#if}`, so a page's reactive children are never torn down. The schedule-picker button is declarative with label `{appState.activeSchedule?.name ?? 'Schedule'}` (the imperative `updateSchedulePickerButton` is gone). Keyboard (Ctrl+Z/Y) via `<svelte:window onkeydown>`; resizable panels + mobile swipe via `onMount`. The 2 live watchers became App `$effect`s (untrack body, first-run-skip for activation) — activation year-sync (minus the dead picker-label half) + page-nav filter-reset.
- New **`src/svelte/SettingsMenu.svelte`** — declarative replacement for `MainController.setupSettingsMenu` (preserves `#settings-menu-btn`/`#settings-theme-btn`/`#settings-undo-btn`/`#settings-redo-btn`; disabled state from `appState.canUndo`/`canRedo`; dropdown is `position:fixed` so rendering inside the header is identical to the old body-append).
- **`main.ts`** now mounts `App` into `#app` + `ModalLayer` separately into `#modal-root` (kept outside `#app` for independent modal stacking), then wires the tutorial + `startApp`. Exposes **`window.services`** (replaces the deleted `window.mainController`).
- **`index.html` `<body>` collapsed** to `<div id="app"></div>` + `<div id="modal-root"></div>` + the Google API scripts + `main.ts`.
- **`UIStateManager.applyPageEffects` retired** (+ the cached `plannerPage`/`schedulePage`/`departmentList`/`courseContainer` els + the empty ctor); `setPage`/`restoreState` drop the `applyPageEffects()` call (App's `style:display` is reactive); `showErrorMessage` now queries `#department-list`/`#course-container` lazily (App mounts after the manager is constructed).
- **`src/ui/controllers/MainController.ts` DELETED** + its `index.ts` barrel export removed.
- tsc 0 / build clean (no new a11y warnings — redundant implicit ARIA roles dropped) / 6-6 tests. Playwright-verified: app boots (81 dept categories, 100 courses), header controls + reactive picker label "My Schedule", page toggle (planner grid ↔ schedule flex, AB1531/AB1532 sidebar + 4 grids), settings menu open/close + disabled states, schedule-picker + filter modals, theme toggle reactive, the **welcome tutorial renders against the new shell** (Step 1/9, demo courses); 0 app console errors. Profile reverted to canonical (My Schedule + AB1531/AB1532 `lec: null`, no Tutorial leftover) — note the tutorial-schedule cleanup is driven by the `appState.activeScheduleId` watcher (relocating in 13D).

### ⬜ 13D — remaining
- **13D** relocate the 4 service-level `watch()` consumers (CourseColorService, AutoScheduleOrchestrator, AppBootstrap data-sub, setupTutorial activeScheduleId) into App.svelte `$effect`s; rewrite `FilterState.svelte.test.ts`; **delete `reactivity.svelte.ts`** + its test; **delete `RUNES_MIGRATION*.md`**.
