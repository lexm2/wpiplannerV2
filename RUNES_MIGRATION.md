...
- ScheduleController: still owns local-event CRUD, section-info modal, color get/set, schedule filter button, and the auto-schedule modal orchestration + `doGenerateSchedules`. Relocate each as a separate 12E unit, THEN delete the controller + index export. MainController shell (Phase 13: App.svelte root, LAST).
