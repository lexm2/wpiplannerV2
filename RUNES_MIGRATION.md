...
- ScheduleController: still owns the auto-schedule modal orchestration + `doGenerateSchedules`. Relocate as the final 12E unit, THEN delete the controller + index export. Also finish the view-refresh teardown (the no-op `refreshCurrentView` + its callers). MainController shell (Phase 13: App.svelte root, LAST).
