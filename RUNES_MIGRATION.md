...
### ⬜ 13C–13D — remaining
- **13C** build `App.svelte` (declarative shell + child components) + `SettingsMenu.svelte`; page display via `style:display`; keyboard via `<svelte:window>`; resizable/swipe via `onMount`; move the 2 live watchers to `$effect`s; **delete MainController** + collapse `index.html`; retire `UIStateManager.applyPageEffects`.
- **13D** relocate the 4 service-level `watch()` consumers (CourseColorService, AutoScheduleOrchestrator, AppBootstrap data-sub, setupTutorial activeScheduleId) into App.svelte `$effect`s; rewrite `FilterState.svelte.test.ts`; **delete `reactivity.svelte.ts`** + its test; expose `window.services`; **delete `RUNES_MIGRATION*.md`**.
