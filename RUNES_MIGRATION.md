...
- ScheduleController: now owns only the wizard methods (`openComponentWizard` + callbacks) + `getIncompleteSelectionInfo`/`hasValidTimeSlot`. Relocate those as the final 12E unit, THEN delete the controller + index export. Also finish the view-refresh teardown (the no-op `refreshCurrentView` + its callers). MainController shell (Phase 13: App.svelte root, LAST).
