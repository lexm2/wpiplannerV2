...
async function walkTutorials(page: Page): Promise<StepResult[]> {
    return page.evaluate(async () => {
        ...
        while (guard++ < 120) {
            ...
            if (key !== lastKey) {
                results.push({ tutorial: active.id, index, title: step.title, selector: step.selector, found, highlighted });
            }
            lastKey = key;
...
test('every tutorial step highlights a real, visible element', async ({ page }) => {
    ...
    const report = broken
        .map((r) => `  ✗ ${r.tutorial}#${r.index} "${r.title}" → ${r.selector}` +
            ` (found=${r.found}, highlighted=${r.highlighted})`)
        .join('\n');
    expect(broken, `Tutorial steps whose target could not be found/highlighted:\n${report}`).toEqual([]);
});
