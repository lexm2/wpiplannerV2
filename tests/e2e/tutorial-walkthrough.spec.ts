...
type StepReport = StepMeta & StepProbe & { advanced: boolean; clickError: string | null };

const STEP_KEY = (m: StepMeta | null) => (m ? `${m.tutorial}#${m.index}` : 'done');
...
async function probeStep(page: Page, selector: string): Promise<StepProbe> {
    return page.evaluate(async (sel) => {
        ...
        if (!el) {
            return {
                matches: 0, visible: false, highlighted: !!svg,
                highlightOwned: false, highlightOverlaps: false, blockedBy: null,
            };
        }
        ...
        const highlightOwned = !!svgParent && (svgParent === el || svgParent === el.parentElement);
        const highlightOverlaps = !!svg && overlaps(svg.getBoundingClientRect(), rect);
        ...
        const x = Math.min(Math.max(hit.left + hit.width / 2, 1), window.innerWidth - 1);
        const y = Math.min(Math.max(hit.top + hit.height / 2, 1), window.innerHeight - 1);
        const atPoint = document.elementFromPoint(x, y);
        const registers = !!atPoint?.closest(sel);

        return { matches, visible, highlighted: !!svg, highlightOwned, highlightOverlaps, blockedBy: registers ? null : describe(atPoint) };
    }, selector);
}
...
async function waitForAdvance(page: Page, from: string, timeoutMs = 12_000): Promise<boolean> {
...
test('every tutorial step can be completed by clicking its own target', async ({ page }) => {
    // Nothing should raise a native dialog, but accept any so the walk can't hang.
    page.on('dialog', (d) => d.accept('Tutorial E2E').catch(() => {}));

    await page.goto('/');
    await page.waitForFunction(() => !!(window as any).services?.tutorial, null, { timeout: 30_000 });
    ...
    await page.waitForFunction(
        () => (window as any).services.tutorial.service.getActiveTutorial()?.id === 'welcome',
        null,
        { timeout: 30_000 }
    );
    ...
    for (let guard = 0; guard < 60; guard++) {
        ...
        const clickSelector = meta.waitFor === 'manual' ? '[data-tutorial-next]' : meta.selector;
        ...
        const advanced = clickError ? false : await waitForAdvance(page, STEP_KEY(meta));
    ...
    const broken = results.filter(
        (r) => r.matches !== 1 || !r.visible || !r.highlighted || !r.highlightOwned
            || !r.highlightOverlaps || r.blockedBy !== null || !r.advanced
    );
    const report = broken
        .map((r) => {
            const why: string[] = [];
            if (r.matches === 0) why.push('selector matched no element');
            if (r.matches > 1) why.push(`selector matched ${r.matches} elements (highlight picks the first)`);
            if (r.matches >= 1 && !r.visible) why.push('element is not visible');
            if (!r.highlighted) why.push('no highlight SVG rendered');
            if (r.highlighted && !r.highlightOwned) why.push('highlight SVG attached to a different element');
            if (r.highlighted && r.highlightOwned && !r.highlightOverlaps) why.push('highlight SVG does not overlap the target');
            if (r.blockedBy) why.push(`click point hits ${r.blockedBy}`);
            if (!r.advanced) why.push(`clicking it did not advance the tutorial${r.clickError ? ` (${r.clickError})` : ''}`);
            return `  ✗ ${r.tutorial}#${r.index} "${r.title}" → ${r.selector}\n      ${why.join('\n      ')}`;
        })
        .join('\n');

    expect(broken, `Tutorial steps that a user could not complete:\n${report}`).toEqual([]);
    expect(results.length, 'all four tutorials should have been walked end to end').toBe(EXPECTED_STEPS);
    ...
    const after = await page.evaluate(() => {
        ...
        return {
            ...
            tutDepartments: s.courseDataService.getAllDepartments()
                .filter((d: any) => d.abbreviation === 'TUT').length,
            tutorialSchedules: s.scheduleManagementService.getAllSchedules()
                .filter((sc: any) => sc.name === 'Tutorial' || sc.name.startsWith('Tutorial (')).length,
        };
    });
    expect(after, 'the finished tutorial should have cleaned up after itself').toEqual({
        visited: 'true',
        tutDepartments: 0,
        tutorialSchedules: 0,
    });
