...
describe('DegreeBucketFilter', () => {
    const sample = [fs('CS', '3013'), fs('CS', '2119'), fs('MA', '1021'), fs('BME', '1004')];

    it('passes through when both lists are empty', () => {
        const c: DegreeBucketFilterCriteria = { allowedDepartments: [], excludedCourses: [] };
        expect(filter.apply(sample, c)).toBe(sample);
    });

    it('drops sections whose department is not allowed', () => {
        const c: DegreeBucketFilterCriteria = { allowedDepartments: ['CS'], excludedCourses: [] };
    ...
    it('drops an excluded course even when its department is allowed', () => {
        const c: DegreeBucketFilterCriteria = { allowedDepartments: ['CS'], excludedCourses: ['CS 2119'] };
        expect(codes(filter.apply(sample, c))).toEqual(['CS 3013']);
    });

    it('applies exclusions with no department restriction', () => {
        const c: DegreeBucketFilterCriteria = { allowedDepartments: [], excludedCourses: ['BME 1004'] };
        expect(codes(filter.apply(sample, c))).toEqual(['CS 3013', 'CS 2119', 'MA 1021']);
    });

    it('expands cross-listed excluded entries to both identities', () => {
        const sections = [fs('CS', '2022'), fs('MA', '2201'), fs('CS', '3013')];
        const c: DegreeBucketFilterCriteria = { allowedDepartments: [], excludedCourses: ['CS 2022/ MA 2201'] };
    ...
    it('isValidCriteria accepts well-formed and rejects malformed input', () => {
        expect(filter.isValidCriteria({ allowedDepartments: ['CS'], excludedCourses: [] })).toBe(true);
        expect(filter.isValidCriteria({ allowedDepartments: 'CS', excludedCourses: [] })).toBe(false);
        expect(filter.isValidCriteria({ allowedDepartments: [1], excludedCourses: [] })).toBe(false);
        expect(filter.isValidCriteria(null)).toBe(false);
    });

    it('getDisplayValue summarizes the bucket', () => {
        expect(filter.getDisplayValue({ allowedDepartments: ['CS', 'MA'], excludedCourses: ['CS 2119'], label: 'CS' }))
