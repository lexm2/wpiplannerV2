...
function group(term: string, discussions = 0, labs = 0): LectureGroup {
    return {
        ...
        compatibleDiscussions: Array.from({ length: discussions }, () => section(term)),
        compatibleLabs: Array.from({ length: labs }, () => section(term)),
    };
}

function course(dept: string, number: string, ay: number, groups: LectureGroup[]): Course {
    return {
        id: `${dept}-${number}-${ay}`,
        number,
        name: `${dept} ${number}`,
...
function departments(...courses: Course[]): Department[] {
    ...
    return [...byDept].map(([abbreviation, cs]) => ({ abbreviation, name: abbreviation, courses: cs }));
}

function planned(code: string, year: number, season: 'Fall' | 'Spring', term: string | null): AppliedCourse {
    ...
    return {
        ...
        isTransfer: false,
        isInProgress: true,
        period: { year, season, term, raw: `${year} ${season} ${term ?? 'Semester'}` },
        satisfies: [],
    };
}
...
function record(courses: AppliedCourse[]): StudentRecord {
    return {
        schemaVersion: 1, major: 'CS', degree: 'BS', startYear: 2025, importedAt: 'x',
        requirements: [], courses, credits: { earned: 0, inProgress: 0, transfer: 0, required: null },
    };
}
...
describe('academicYearForPeriod', () => {
    it('Fall keeps the calendar year', () => {
        expect(academicYearForPeriod({ year: 2026, season: 'Fall', term: 'A', raw: '' })).toBe(2026);
    });
    it('Spring maps to the previous fall year', () => {
        expect(academicYearForPeriod({ year: 2027, season: 'Spring', term: 'D', raw: '' })).toBe(2026);
    });
    it('null period -> null', () => {
...
describe('matchPlannedCourses', () => {
    it('auto-selects the section when the planned term has exactly one lecture group', () => {
        ...
        const res = matchPlannedCourses(record([planned('CS 2223', 2026, 'Fall', 'B')]), depts);

        expect(res.stats).toMatchObject({ matched: 1, autoSectioned: 1, pinnedOnly: 0, unmatched: [] });
    ...
    it('pins the term but leaves the section empty when the term is ambiguous', () => {
        const depts = departments(course('CS', '2022', 2026, [group('A'), group('A')]));
        const res = matchPlannedCourses(record([planned('CS 2022/ MA 2201', 2026, 'Fall', 'A')]), depts);
    ...
    it('maps Spring terms to the previous fall academic year', () => {
        ...
        const res = matchPlannedCourses(record([planned('CS 4341', 2027, 'Spring', 'D')]), depts);
    ...
    it('reports courses absent from the catalog as unmatched', () => {
        ...
        const res = matchPlannedCourses(record([planned('CS 9999', 2026, 'Fall', 'B')]), depts);
    ...
    it('ignores completed/transfer courses (only planned are matched)', () => {
        const depts = departments(course('CS', '2223', 2026, [group('B')]));
        const completed: AppliedCourse = { ...planned('CS 2223', 2026, 'Fall', 'B'), isInProgress: false, grade: 'A' };
