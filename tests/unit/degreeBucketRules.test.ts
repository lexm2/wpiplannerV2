...
const VALID_CATEGORIES = new Set([
    'total_credits', 'residency', 'mqp', 'mqp_completion', 'iqp', 'iqp_completion', 'hua',
    'hua_completion', 'social_science', 'physical_education', 'major_specific', 'free_electives', 'unused',
]);
...
describe('degreeBucketRules lookup', () => {
    it('merges WPI-wide defaults with the major (major wins on category+label)', () => {
        ...
        expect(rules.some(r => r.category === 'major_specific' && r.validDepartments.includes('CS'))).toBe(true);
    });
    ...
    it('resolves the newest class year <= the requested one', () => {
        ...
        const sci = (year: number) =>
            getSheetRules('Computer Science', year)
                .find(r => r.category === 'major_specific' && r.label.includes('BASIC SCIENCE'));
    ...
    it('returns the CS requirement exclusion (CS 2119)', () => {
        const rule = getBucketRule('Computer Science', 2030, 'major_specific', 'Computer Science');
    ...
    it('produces filter criteria from a bucket rule', () => {
        const criteria = getDegreeBucketCriteria('Computer Science', 2030, 'social_science');
    ...
    it('resolves major-specific rules through MAJOR_ALIASES (runtime name -> sheet key)', () => {
        ...
        const aliased = getSheetRules('Applied Physics', 2029)
            .filter(r => r.category === 'major_specific');
        const direct = getSheetRules('Physics Applied', 2029)
            .filter(r => r.category === 'major_specific');
    ...
    it('returns null for an unknown major + non-shared category', () => {
        expect(getDegreeBucketCriteria('Nonexistent Major', 2030, 'major_specific')).toBeNull();
    });

    it('falls back to WPI-wide for an unknown major on a shared category', () => {
        const criteria = getDegreeBucketCriteria('Nonexistent Major', 2030, 'social_science');
        expect(criteria).not.toBeNull();
...
describe('degreeBucketRules constants integrity', () => {
    const allRules: DegreeBucketRule[] = [
        ...WPI_WIDE_RULES,
        ...Object.values(MAJOR_RULES).flatMap(byYear => Object.values(byYear).flat()),
    ];
    ...
    it('every category is a valid RequirementCategory', () => {
        for (const r of allRules) expect(VALID_CATEGORIES.has(r.category)).toBe(true);
    });
    ...
    it('every excluded course parses to a DEPT NUMBER', () => {
        for (const r of allRules)
            for (const code of r.excludedCourses) expect(candidateCodes(code).length).toBeGreaterThan(0);
    });
});
