...
import { sectionsOf, encodeCourseSelection, decodeCourseSelection } from '../../src/utils/courseUtils';
import { applyPreviewOverlay, buildHoverCourse } from '../../src/svelte/schedule/scheduleGeometry';
...
function course(id: string, opts: { standalone?: boolean } = {}): Course {
    return {
        ...
        ...(opts.standalone
            ? { standaloneLabs: [LAB] }
            : { lectures: [{ section: LEC, compatibleDiscussions: [DIS], compatibleLabs: [LAB] }] }),
    } as unknown as Course;
}

function selectedCourse(id: string, selected: SelectedCourse['selected']): SelectedCourse {
    return { course: course(id), selected, isRequired: false, lockedSections: new Set() };
}
...
describe('sectionsOf', () => {
    it('returns lecture, then discussion, then lab', () => {
        expect(sectionsOf({ lecture: LEC, discussion: DIS, lab: LAB })).toEqual([LEC, DIS, LAB]);
    });
...
describe('export encoding', () => {
    it('emits a fixed positional 4-tuple regardless of key insertion order', () => {
        ...
        expect(encodeCourseSelection(selectedCourse('CH-1010', built))).toEqual([
            'CH-1010', '1001', null, '1003',
        ]);
    });

    it('round-trips through decode, dropping the kinds that were null', () => {
        const c = course('CH-1010');
        const [, lec, dis, lab] = encodeCourseSelection(selectedCourse('CH-1010', { lecture: LEC, lab: LAB }));
...
describe('applyPreviewOverlay', () => {
    const existing = selectedCourse('CH-1010', { lecture: LEC, discussion: DIS, lab: LAB });
    ...
    it('replaces the whole map, so kinds the wizard dropped disappear', () => {
        const [out] = applyPreviewOverlay([existing], existing.course, { lecture: LEC });
    ...
    it('passes the list straight through with nothing to preview', () => {
        expect(applyPreviewOverlay([existing], null, { lecture: LEC })).toEqual([existing]);
        expect(applyPreviewOverlay([existing], existing.course, null)).toEqual([existing]);
    });
});
...
describe('buildHoverCourse', () => {
    ...
    it('carries the base course with only the hovered kind', () => {
        const hovered = buildHoverCourse([existing], existing.course, { discussion: DIS });
