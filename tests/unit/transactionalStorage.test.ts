...
describe('preferences round-trip', () => {
  it('saves and loads preferences', () => {
    const m = mgr();
    const res = m.savePreferences({
      theme: 'wpi-light',
      bookmarkedCourseIds: ['CS1101'],
    });
  ...
  it('writes to the shared registry key, not an ad-hoc literal', () => {
    ...
    expect(localStorage.getItem(STORAGE_KEYS.PREFERENCES)).toContain(
      'wpi-dark',
    );
  });
...
describe('degree record', () => {
  it('round-trips a record and clears on null', () => {
    ...
    expect(m.loadDegreeRecord().data).toMatchObject({
      major: 'Computer Science',
    });
...
describe('rollback', () => {
  it('restores every touched key when an operation throws mid-transaction', () => {
    ...
    const result = (
      m as unknown as {
        executeSyncTransaction: (op: () => void) => {
          success: boolean;
          rolledBack?: boolean;
        };
      }
    ...
    expect(localStorage.getItem(STORAGE_KEYS.ACTIVE_SCHEDULE_ID)).toBe(
      before.active,
    );
  });
