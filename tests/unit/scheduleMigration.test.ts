...
const row = (name: string) =>
  rows.find(r => (r.schedule as any).name === name)!;
...
function migrate(fixtureRow: Row, version?: number): Record<string, any> {
  return migrateStoredSchedule(revive(fixtureRow), version) as Record<
    string,
    any
  >;
}
...
describe('migrateStoredSchedule', () => {
  it('preserves every component selection in a real captured profile', () => {
    ...
    const numbers = (sc: any) =>
      Object.fromEntries(
        Object.entries(sc.selected).map(([k, v]: any) => [k, v.number]),
      );
  ...
  describe('robustness', () => {
    ...
    it('never throws on input it cannot understand', () => {
      ...
      expect(
        migrateStoredSchedule({ selectedCourses: [null, 7] } as any),
      ).toEqual({
  ...
  it('round-trips through the Set replacer/reviver unchanged', () => {
    for (const row of rows) {
      ...
      const roundTripped = JSON.parse(
        JSON.stringify(migrated, setReplacer),
        setReviver,
      );
